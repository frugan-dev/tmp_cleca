...
$Mod = $this->container->get('Mod\Catmember\\'.ucfirst((string) $this->env));
...
if (isset($this->Mod->postData[$key])) {
    ...
    $eventName = 'event.'.$this->env.'.'.$Mod->modName.'.getOne.where';
    ...
    if (!empty($row['id'])) {
        $params['value'] = $this->Mod->{$key} = $row['id'];
    }
}
...
// if (!$this->rbac->isGranted($Mod->modName.'.'.$this->env.'.add')) {
...
$result = $Mod->getAll([
    'order' => (!empty($Mod->fields['name']['multilang']) ? 'b' : 'a').'.name ASC',
]);
...
if ((is_countable($result) ? count($result) : 0) > 0) {
    foreach ($result as $row) {
        ...
        if (!isset($this->Mod->postData[$key]) && 1 === (is_countable($result) ? count($result) : 0)) {
            $params['value'] = $row['id'];
        }
    }
}
