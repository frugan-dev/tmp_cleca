...
if ('' !== trim((string) $this->Mod->multilang[$langId][$key] ?? '')) {
    echo '<div class="d-flex mb-3">'.PHP_EOL;

    echo '<div class="flex-shrink-0">'.PHP_EOL;
    echo '<a class="d-inline-block" title=""'.$this->escapeAttr([
        'href' => $this->asset('media/file/'.$this->controller.'/'.$langRow['isoCode'].'/'.$this->Mod->multilang[$langId][$key]),
    ]).'>'.$this->escape()->html($this->Mod->multilang[$langId][$key]).'</a>'.PHP_EOL;
    echo '</div>'.PHP_EOL;
    ...
    if (empty($val[$this->env]['attr']['required'])) {
        ...
        echo $this->helper->Html()->getFormField([
            ...
            'attr' => [
                'type' => 'checkbox',
                'id' => 'multilang-'.$langId.'-unlink-'.$key,
                'name' => 'multilang|'.$langId.'|unlink_'.$key,
                'value' => 1,
                'class' => ['form-check-input'],
            ],
        ]);

        echo '<label class="form-check-label"'.$this->escapeAttr(['for' => $this->helper->Nette()->Strings()->webalize('multilang-'.$langId.'-unlink-'.$key)]).'>'.$this->escape()->html(__('delete')).'</label>'.PHP_EOL;
...
if (!empty($val[$this->env]['attr']['required']) && !empty($this->Mod->multilang[$langId][$key])) {
