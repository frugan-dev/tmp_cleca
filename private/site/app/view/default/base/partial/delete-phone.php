...
$row = $this->container->get('Mod\Country\\'.ucfirst((string) $this->env))->getOne([
    'id' => (int) $this->Mod->{$key.'_country_id'},
]);

echo !empty($row) && '' !== trim((string) $this->Mod->{$key}) ? '<a title=""'.$this->escapeAttr([
    'href' => 'tel:'.$this->helper->Strings()->phoneUri($row['phone_code'].$this->Mod->{$key}),
]).'>'.$this->escape()->html('+'.$row['phone_code'].' '.$this->Mod->{$key}).'</a>' : '&nbsp;';
