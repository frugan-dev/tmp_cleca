...
if (!empty(${'cat'.$Mod->modName.'Id'})) {
    ...
    if ((is_countable($result) ? count($result) : 0) > 0) {
        foreach ($result as $n => $row) {
            $params['options'][$row['id']] = $this->escape()->html(++$n.' - '.$row['name']);
        }
    }
} else {
            ...
            '" + cat'.$Mod->modName.'Id',
                ...
                .then(data => {
                    ...
                    for (var i = 0; i < data.response.length; i++) {
                        ...
                        '.$Mod->modName.'IdEl.insertAdjacentHTML("beforeend", "<option value=\"" + data.response[i].id + "\">" + (i + 1) + " - " + data.response[i].name + "</option>");
                    }
