...
if ('' !== trim((string) ($this->Mod->multilang[$langId][$key]['value'] ?? ''))) {
    echo '<a title="" href="'.$this->asset('media/file/'.$this->controller.'/'.$langRow['isoCode'].'/'.$this->Mod->multilang[$langId][$key]['value']).'">'.PHP_EOL;
    echo $this->escape()->html($this->Mod->multilang[$langId][$key]['value']);
    echo '</a>'.PHP_EOL;
} else {
    echo '&nbsp;';
}
