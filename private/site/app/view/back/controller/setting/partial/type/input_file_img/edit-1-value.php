...
$params = [
    ...
    'attr' => [
        ...
        'accept' => implode(',', array_values($this->config['mime.img.allowedTypes'])),
        'data-type' => 'img',
        'data-maxFileSize' => $this->helper->File()->getBytes($this->config['mod.'.$this->controller.'.img.uploadMaxFilesize'] ?? $this->config['media.img.uploadMaxFilesize'] ?? \Safe\ini_get('upload_max_filesize')),
        'data-minWidth' => $this->config['mod.'.$this->controller.'.img.minWidth'] ?? $this->config['media.img.minWidth'],
        'data-minHeight' => $this->config['mod.'.$this->controller.'.img.minHeight'] ?? $this->config['media.img.minHeight'],
        'lang' => $this->lang->code,
        // 'data-required' => true,
    ],
    ...
    'help' => [
        '<b>'.sprintf(
            __('Recommended width: %1$d pixel.'),
            1000
        ).'</b>',
        sprintf(
            __('Minimum dimensions: %1$dx%2$d pixel.'),
            $this->config['mod.'.$this->controller.'.img.minWidth'] ?? $this->config['media.img.minWidth'],
            $this->config['mod.'.$this->controller.'.img.minHeight'] ?? $this->config['media.img.minHeight']
        ),
        sprintf(
            __('Allowed extensions: %1$s.'),
            implode(', ', $this->helper->Arrays()->getExtFromConfig($this->config['mime.img.allowedTypes']))
        ),
        sprintf(
            __('Maximum weight per file: %1$s.'),
            $this->helper->File()->formatSize($this->config['mod.'.$this->controller.'.img.uploadMaxFilesize'] ?? $this->config['media.img.uploadMaxFilesize'] ?? \Safe\ini_get('upload_max_filesize'))
        ),
    ],
];
...
if ($this->Mod->type === basename(__DIR__) && '' !== trim((string) ($this->Mod->{$key}['value'] ?? ''))) {
    echo '<div class="d-flex mb-3">'.PHP_EOL;

    echo '<a data-fancybox title=""'.$this->escapeAttr([
        'href' => $this->asset('media/img/'.$this->controller.'/lg/'.$this->Mod->{$key}['value']),
    ]).'>'.PHP_EOL;
    echo '<img class="img-thumbnail" alt=""'.$this->escapeAttr([
        'src' => $this->asset('media/img/'.$this->controller.'/xs/'.$this->Mod->{$key}['value']),
    ]).'>'.PHP_EOL;
    echo '</a>'.PHP_EOL;
    ...
    if (empty($this->Mod->required)) {
        ...
        echo $this->helper->Html()->getFormField([
            ...
            'attr' => [
                'type' => 'checkbox',
                'id' => 'unlink-'.$key.'-value',
                'name' => 'unlink_'.$key.'[value]',
                'value' => 1,
                'class' => ['form-check-input'],
            ],
        ]);

        echo '<label class="form-check-label"'.$this->escapeAttr(['for' => $this->helper->Nette()->Strings()->webalize('unlink-'.$key.'-value')]).'>'.$this->escape()->html(__('delete')).'</label>'.PHP_EOL;
