...
{
    use V1ActionDeleteTrait;
    use V1ActionDeleteXhrTrait;
    use V1ActionGetTrait;
    use V1ActionGetXhrTrait;
    use V1ActionPostTrait;
    use V1ActionPostXhrTrait;
    use V1ActionPutTrait;
    use V1ActionPutXhrTrait;
    ...
    public function __invoke(RequestInterface $request, ResponseInterface $response, $args)
    {
        try {
            ...
            $this->version = $args['v'] ?? $this->config['api.version'];
            ...
            if (!empty($this->controller) && !empty($this->actionCamelCase)) {
                if (method_exists($this, 'v'.$this->version.'Action'.$this->methodCamelCase.$this->xhrCamelCase) && \is_callable([$this, 'v'.$this->version.'Action'.$this->methodCamelCase.$this->xhrCamelCase])) {
                    if (!empty($request->getHeaderLine($this->config['api.headers.key'])) && !empty($this->auth->getIdentity()[$this->auth->getIdentity()['_role_type'].'_api_log_level'])) {
                        $logLevel = !empty($request->getHeaderLine($this->config['api.headers.key'])) ? $this->logger::toMonologLevel($this->auth->getIdentity()[$this->auth->getIdentity()['_role_type'].'_api_log_level'])->toPsrLogLevel() : 'debug';

                        \call_user_func_array(
                            [$this->logger, $logLevel],
                            [
                                \sprintf(_x('Rest API %1$s %2$s', $this->context, $this->config['logger.locale']), $request->getMethod(), $this->helper->Url()->getPathUrl()),
                            ]
                        );
    ...
    public function init(): void
    {
        ...
        if ($this->container->has('request')) {
            // TODO - use https://github.com/slimphp/Slim-Http/#serverrequestisxhr
            if ('text/html' === $this->request->getHeaderLine('Accept') && !empty((bool) ('XMLHttpRequest' === $this->request->getHeaderLine('X-Requested-With')))) {
                static::$env = 'front';
            }
        }
