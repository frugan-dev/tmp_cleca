...
if ('' !== trim((string) $this->Mod->multilang[$langId][$key])) {
    echo '<a title="" href="'.$this->asset('media/file/'.$this->controller.'/'.$langRow['isoCode'].'/'.$this->Mod->multilang[$langId][$key]).'">'.PHP_EOL;
    echo $this->escape()->html($this->Mod->multilang[$langId][$key]);
    echo '</a>'.PHP_EOL;
} else {
    echo '&nbsp;';
}
