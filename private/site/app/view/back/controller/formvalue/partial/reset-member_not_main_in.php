...
$eventName = 'event.'.$this->env.'.'.$ModCatmember->modName.'.getAll.where';
$callback = function (GenericEvent $event) use ($ModCatmember): void {
    $ModCatmember->dbData['sql'] .= ' AND a.main = :main';
    $ModCatmember->dbData['args']['main'] = 0;
};

$this->dispatcher->addListener($eventName, $callback);

$result = $ModCatmember->getAll([
    'order' => (!empty($ModCatmember->fields['name']['multilang']) ? 'b' : 'a').'.name ASC',
]);
...
if ((is_countable($result) ? count($result) : 0) > 0) {
    foreach ($result as $row) {
        $names[] = $this->escape()->html($row['name']);
    }
}

echo '<label class="form-check-label"'.$this->escapeAttr(['for' => $key]).'>'.$this->escape()->html(sprintf(_x('Delete all non-main %1$s who have filled out some %2$s in the selected %3$s.', $this->container->get('Mod\Member\\'.ucfirst((string) $this->env))->context), $this->helper->Nette()->Strings()->lower($this->container->get('Mod\Member\\'.ucfirst((string) $this->env))->pluralName).' ('.implode(', ', $names).')', $this->helper->Nette()->Strings()->lower($this->container->get('Mod\Form\\'.ucfirst((string) $this->env))->pluralName), $this->helper->Nette()->Strings()->lower($this->container->get('Mod\Catform\\'.ucfirst((string) $this->env))->singularName))).'</label>'.PHP_EOL;

__('Delete all non-main %1$s who have filled out some %2$s in the selected %3$s.', 'default');
__('Delete all non-main %1$s who have filled out some %2$s in the selected %3$s.', 'male');
__('Delete all non-main %1$s who have filled out some %2$s in the selected %3$s.', 'female');
