...
$Mod = $this->container->get('Mod\Catform\\'.ucfirst((string) $this->env));
...
if (($paramId = array_search('status_id', $routeParamsArr, true)) !== false) {
    if (isset($routeParamsArr[$paramId + 1])) {
        unset($routeParamsArr[$paramId], $routeParamsArr[$paramId + 1]);
    }
}
...
if ((is_countable($result) ? count($result) : 0) > 0) {
    foreach ($result as $row) {
        ...
        $params['options'][$this->uri([
            ...
            'value' => $this->escape()->html($this->helper->Nette()->Strings()->truncate($row['code'].' - '.$row['name'], 50)),
        ];
    }
}
