...
echo '' !== trim((string) $this->Mod->{$key}['value']) ? '<a title=""'.$this->escapeAttr([
    'href' => 'tel:'.$this->helper->Strings()->phoneUri($this->Mod->{$key}['value']),
]).'>'.$this->escape()->html($this->Mod->{$key}['value']).'</a>' : '&nbsp;';
