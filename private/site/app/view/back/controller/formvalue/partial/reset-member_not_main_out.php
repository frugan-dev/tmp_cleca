...
echo '<label class="form-check-label"'.$this->escapeAttr(['for' => $key]).'>'.$this->escape()->html(sprintf(_x('Delete all non-main %1$s who have not filled in any %2$s in any %3$s.', $this->container->get('Mod\Member\\'.ucfirst((string) $this->env))->context), $this->helper->Nette()->Strings()->lower($this->container->get('Mod\Member\\'.ucfirst((string) $this->env))->pluralName).' ('.implode(', ', $names).')', $this->helper->Nette()->Strings()->lower($this->container->get('Mod\Form\\'.ucfirst((string) $this->env))->singularName), $this->helper->Nette()->Strings()->lower($this->container->get('Mod\Catform\\'.ucfirst((string) $this->env))->singularName))).'</label>'.PHP_EOL;

__('Delete all non-main %1$s who have not filled in any %2$s in any %3$s.', 'default');
__('Delete all non-main %1$s who have not filled in any %2$s in any %3$s.', 'male');
__('Delete all non-main %1$s who have not filled in any %2$s in any %3$s.', 'female');
