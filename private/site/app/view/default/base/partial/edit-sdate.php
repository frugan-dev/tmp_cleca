...
echo '</label>'.PHP_EOL;
echo '<div class="col-10 col-sm-7 col-lg-5">'.PHP_EOL; // <--
...
if (isset($this->Mod->postData[$key])) {
    $this->Mod->postData[$key] = $this->helper->Carbon()->createFromFormat('Y-m-d H:i:s', $this->Mod->postData[$key], $this->config['db.1.timeZone'])->setTimezone(date_default_timezone_get())->format('Y-m-d\TH:i');

    $params['value'] = $this->Mod->postData[$key];
    $params['attr']['value'] = $this->Mod->postData[$key];
} else {
    if (!empty($this->Mod->{$key})) {
        $this->Mod->{$key} = $this->helper->Carbon()->createFromFormat('Y-m-d H:i:s', $this->Mod->{$key}, $this->config['db.1.timeZone'])->setTimezone(date_default_timezone_get())->format('Y-m-d\TH:i');
    }

    $params['value'] = $this->Mod->{$key};
    $params['attr']['value'] = $this->Mod->{$key};
}
