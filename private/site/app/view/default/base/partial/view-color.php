...
if ('' !== trim((string) $this->Mod->{$key})) {
    echo '<span'.$this->escapeAttr([
        'class' => array_merge(['badge'], $this->helper->Color()->contrast($this->Mod->{$key}) ? ['text-white'] : ['text-dark']),
        'style' => 'background-color:'.$this->Mod->{$key},
    ]).'>'.$this->escape()->html($this->Mod->{$key}).'</span>';
} else {
    echo '&nbsp;';
}
