...
if ($this->Mod->type === basename(__DIR__) && '' !== trim((string) ($this->Mod->multilang[$langId][$key]['value'] ?? ''))) {
    ...
    echo '<a data-fancybox title=""'.$this->escapeAttr([
        'href' => $this->asset('media/img/'.$this->controller.'/'.$langRow['isoCode'].'/lg/'.$this->Mod->multilang[$langId][$key]['value']),
    ]).'>'.PHP_EOL;
    echo '<img class="img-thumbnail" alt=""'.$this->escapeAttr([
        'src' => $this->asset('media/img/'.$this->controller.'/'.$langRow['isoCode'].'/xs/'.$this->Mod->multilang[$langId][$key]['value']),
    ]).'>'.PHP_EOL;
    echo '</a>'.PHP_EOL;
