...
// Short-circuiting https://www.phptutorial.net/php-tutorial/php-and-operator
if (!empty($row[$this->controller.'value_data']) && !empty($rowCountry = $this->container->get('Mod\Country\\'.ucfirst((string) $this->env))->getOne([
    'id' => $row[$this->controller.'value_data'],
    'active' => true,
]))) {
    echo $this->escape()->html($rowCountry['name']);
} else {
    echo '&nbsp;';
}
