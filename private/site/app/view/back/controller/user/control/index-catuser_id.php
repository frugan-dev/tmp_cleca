...
$result = $this->container->get('Mod\Catuser\\'.ucfirst((string) $this->env))->getAll([
    'order' => (!empty($this->container->get('Mod\Catuser\\'.ucfirst((string) $this->env))->fields['hierarchy']) ? 'a.hierarchy ASC, ' : '').(!empty($this->container->get('Mod\Catuser\\'.ucfirst((string) $this->env))->fields['name']['multilang']) ? 'b' : 'a').'.name ASC',
]);
