...
$params['options'][0] = '- '._x('none', $this->Mod->context).' -'; // <--

__('none', 'default');
__('none', 'male');
__('none', 'female');

$result = $this->Mod->getTree([
    'order' => (!empty($this->Mod->fields['hierarchy']) ? 'a.hierarchy ASC, ' : '').(!empty($this->Mod->fields['name']['multilang']) ? 'b' : 'a').'.name ASC',
    'field' => (!empty($this->Mod->fields['name']) ? 'name' : 'id'),
]);

if ((is_countable($result) ? count($result) : 0) > 0) {
    $maxLevel = $this->Mod->getMaxLevel();

    foreach ($result as $row) {
        $params['options'][$row['id']] = [
            'attr' => [
                'value' => $row['id'],
            ],
            'value' => $this->escape()->html($row['field']),
        ];

        if ($row['id'] === $this->Mod->id
            || $row['parent_id'] === $this->Mod->id
            || in_array($this->Mod->id, $row['parentIds'], true)
            || ($row['level'] + $maxLevel) >= ($this->config['mod.'.$this->controller.'.tree.maxLevel'] ?? $this->config['mod.tree.maxLevel'] ?? 0)) {
            $params['options'][$row['id']]['attr']['disabled'] = true;
        }
    }
}
